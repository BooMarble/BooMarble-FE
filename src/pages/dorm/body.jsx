...
function Body() {
  ...
  useEffect(() => {
    ...
    const fetchDetailInfo = async () => {
      try {
        const response = await axios.get(`https://boomarble.com/reviews/${universityId}/dorm`,
  ...
  if (isLoading) {
    ...
    return(
        <Container>
              <h1>기숙사 & 숙소</h1>
              <DetailContainer>
              {detailInfo.reviewDormDTOList.map((review, index) => (
             <DetailBox key={index}>
             <h2>{review.writer.nickname}</h2>
             <p>{review.dormitoryName}</p>
             <p>{review.dormitoryDesc}</p>
             </DetailBox>
        ))}
      </DetailContainer>
        </Container>
